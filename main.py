...
def main():
    ...
    die_1 = roll_die()
    die_2 = roll_die()
    decide = get_question_type()
    print_question(die_1, die_2, decide)
    user = input_answer()
    if user == "":
        print_error_message()
    else:
        correct = is_correct_answer(die_1, die_2, decide, user)
        if correct is True:
            print_congratulations(decide)
        else:
            print_correct_answer(die_1, die_2, decide)
